...
func (r *Result) FUint() uint64 {
	switch r.Data.(type) {
	case string:
		v, err := strconv.ParseUint(r.Data.(string), 10, 64)
		if err != nil {
			panic(err)
		}
		return v
	case float32:
		return uint64(r.Data.(float32))
	case float64:
		return uint64(r.Data.(float64))
	case int:
		return uint64(r.Data.(int))
	case int8:
		return uint64(r.Data.(int8))
	case int16:
		return uint64(r.Data.(int16))
	case int32:
		return uint64(r.Data.(int32))
	case int64:
		return uint64(r.Data.(int64))
	case uint:
		return uint64(r.Data.(uint))
	case uint8:
		return uint64(r.Data.(uint8))
	case uint16:
		return uint64(r.Data.(uint16))
	case uint32:
		return uint64(r.Data.(uint32))
	case uint64:
		return r.Data.(uint64)
	default:
		panic(failedFormat)
	}
}
